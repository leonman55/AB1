...
public class FirstGame implements Game, ExerciseSubmission {
    	...
    	/**
    	 * Builds power lines & plants for given Scenario
    	 */
	@Override
	...
	private void buildWindEnergyPlants(Scenario scenario) {
		...
		// Sort on basis of construction cost decreasingly. Expensive element first.
		...
		while (!sortedStack.isEmpty() && !overBudget && countBuilt > 0) {
			...
			// Skip if no transformer station is found on the playfield element.
			if (target != null) {
	...
	private void buildPowerLines(Scenario scenario) {
		...
		// Create sorted set with comparator to sort power lines according to their costs for upgrading to HIGH_VOLTAGE
