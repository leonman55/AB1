...
public class EnergyNetworkAnalyzerImpl extends AbstractEnergyNetworkAnalyzer implements ExerciseSubmission {
	 ...
	 * 
	 * @param graph Flow network represented by a {@link Graph}.
	 * @param producerCapacities Capacities of all producers represented by a {@link Map}.
	 * @param consumerCapacities Capacities of all consumers represented by a {@link Map}.
	 */
	...
	/**
	 * Creates a FlowGraph out of given Capacities (producer & consumer) & given Graph
	 * 
	 * @param graph Flow network represented by a {@link Graph}.
	 * @param producerCapacities Capacities of all producers represented by a {@link Map}.
	 * @param consumerCapacities Capacities of all consumers represented by a {@link Map}.
	 * @return A {@link FlowGraph} of {@link EnergyNode}
	 */
	@Override
	public FlowGraph<EnergyNode> createFlowGraph(Graph<EnergyNode, PowerLine> graph, Optional<Map<Producer, Integer>> producerCapacities, Optional<Map<Consumer, Integer>> consumerCapacities) {
		...
		for (PowerLine line : graph.getEdges()) {
			// Add edge in each direction with equal capacity to represent undirected edge of energy graph in flow graph
		...
		// Connect the super source to each producer with new directed edge. The capacity is equal to the currently provided energy level if no artificial capacities are provided.
		for (Producer prod : new ScenarioUtilImpl().getProducers(graph)) {
			if (producerCapacities.isPresent()) {
				...
				flowGraph.addEdge(super_source, prod, prod.getProvidedPower());
			}
		}

		// Connect each consumer to super sink with new directed edge to super source. The capacity is equal to the currently required energy level if no artificial capacities are provided.
		for (Consumer cons : new ScenarioUtilImpl().getConsumers(graph)) {
			if (consumerCapacities.isPresent()) {
				...
				flowGraph.addEdge(cons, super_sink, cons.getRequiredPower());
			}
		}
	...
	public void calculateMaxFlow() {
		...
		for (FlowEdge<EnergyNode> edge : flowGraph.edgesFrom(super_source)) {
			...
			// Update level for producer, casting is safe here due to prior check of existence of node in map
		...
		for (FlowEdge<EnergyNode> edge : flowGraph.getEdges()) {
			...
			if (edge.getEnd().equals(super_sink)) {
				...
				// Update level for consumer, casting is safe here due to prior check of existence of node in map
