...
public class FordFulkersonImpl<V> implements FordFulkerson<V>, ExerciseSubmission {
	...
	/**
	 * Computes the maximum flow in a flow network <code>graph</code> from source <code>s</code> to target <code>t</code>.
	 * 
	 * @param graph The {@link FlowGraph} representing the flow network. Flows along edges in this graph will be set to max flows.
	...
	public void findMaxFlow(FlowGraph<V> graph, V start, V target) {
		...
		// Start iterating as long as there does a path exists with remaining capacity
		while (true) {
		...
		for (V node : residualGraph.getNodes()) {
			for (ResidualEdge<V> edge : residualGraph.edgesFrom(node)) {
				...
				// Get edge running from end to start as capacity of residual non-original edge indicates flow of edge
				FlowEdge<V> flowEdge = graph.getEdge(edge.getEnd(), edge.getStart());

				// For undirected edges get reverse edge in flow graph, null otherwise
				...
				if (rev_flowEdge != null && flowEdge != null) {
					...
					// If there is a reverse edge in the flow graph, flow can be derived from corresponding edge in residual graph.
	...
	/**
	 * Calculates the shortest path on the graph by using breadth-first search (BFS)
	 * 
	 * @param start The start node of the path.
	 * @param end The end node of the path.
	 * @param graph The {@link ResidualGraph} to search in.
	 * @return A path (represented by a {@link Deque}) containing all the underlying edges on the shortest path
	 */
	@Override
	 ...
	 * 
	 * @param path The path (represented by a {@link Deque}) to be augmented.
	 */
	@Override
